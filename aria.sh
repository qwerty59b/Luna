...
aria2c --allow-overwrite=true --auto-file-renaming=true --bt-enable-lpd=true --bt-detach-seed-only=true \
       --bt-remove-unselected-file=true --bt-tracker="[$tracker_list]" --bt-max-peers=50 --enable-rpc=true \
       --rpc-max-request-size=1024M --max-connection-per-server=5 --max-concurrent-downloads=1 --split=10 \
       ...
       --http-accept-gzip=true --max-file-not-found=0 --max-tries=10 --peer-id-prefix=-qB4520- --reuse-uri=true \
